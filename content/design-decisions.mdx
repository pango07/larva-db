...
A construct joins the dialect when **agents writing conservative SQL emit it routinely AND it executes within the existing engine shape** — in-memory algebra after pruning, one atomic commit per write. `HAVING`, `DISTINCT`, scalar functions, `CASE`, upsert, sequences, and composite uniques cleared it. Subqueries never will, regardless of demand — they'd change the engine's shape, and the honest answer to needing them is `larva export`.
