...
| **5** | [exact decimal columns](/docs/sql#exact-decimals--tdecimalscale) (`t.decimal(scale)`): values are canonical strings compared and aggregated with BigInt. A format bump because an older client would order them **lexicographically** — silently wrong results, the exact hazard this guard exists for. A store only declares 5 when its schema actually contains a decimal column; since format ≥ 3 means log mode, decimal stores are always on the commit log (CAS-mode stores need one `db.upgrade()` first, and new stores with decimal schemas are simply born there) |
