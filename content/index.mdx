...
- **No silently lost writes, ever.** Every commit goes through an atomic protocol; conflicts rebase, re-execute, or fail loudly. Verified by 343 checks including randomized concurrent-writer gauntlets.
...
- **Exact money** — `t.decimal(scale)` computes with arbitrary-precision integers: `SUM` never drifts, equality never misfires, and nothing rounds silently. The number on the dashboard is the number that was entered.
