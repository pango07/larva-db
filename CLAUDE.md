...
- On-store format versioning (Design §5): formats 1 (original), 2 (sequences + composite uniques), 3 (ordered commit log). Manifests declare the minimum format their content requires; clients refuse newer formats loudly (`FormatError`). Any future format bump ships with an explicit atomic `upgrade()`; `rollbackTo` preserves the format version.
...
- **Explicitly not supported**: subqueries, window functions, `UNION`, self-joins, 3+ table joins, `ALTER TABLE`, views, triggers, secondary indexes. Each exclusion is deliberate (Design §7, §14). The bar for adding a construct: agents writing conservative SQL emit it routinely AND it executes within the existing engine shape (in-memory algebra after pruning, one CAS per write) — `HAVING`, `DISTINCT`, scalar functions, `CASE`, and `ON CONFLICT` upsert cleared it; subqueries never will.
