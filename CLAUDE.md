...
- On-store format versioning (Design §5): formats 1 (original), 2 (sequences, auto-UUID columns + composite uniques), 3 (ordered commit log), 4 (two-tier writes: intent queues + durable-at-PUT appends), 5 (exact decimal columns — canonical strings + BigInt; implies log mode, so CAS stores need `upgrade()` first and decimal-schema stores are born on the log). Manifests declare the minimum format their content requires (`requiredFormatVersion`); clients refuse newer formats loudly (`FormatError`), including mid-log format transitions (entry-level check + forced checkpoint). `upgrade()` targets `TOP_PROTOCOL_FORMAT` (4) raised by schema demands — a store never locks out old clients for a feature it doesn't use. `rollbackTo` preserves the format version.
...
- **Performance envelope is stated honestly, on purpose**: single-digit commits/sec (all commits serialize through one CAS), per-query cost proportional to chunks touched, pruning on the primary key, the one `partitionBy()` column, and `.index()`ed columns (secondary index blobs, chunkId-keyed so staleness degrades pruning, never correctness — no format bump). Don't add complexity to push past these ceilings — the answer to outgrowing Larva is `larva export`.
- **Explicitly not supported**: correlated subqueries, derived tables (subqueries in FROM), window functions, `UNION`, `RIGHT`/`FULL`/`CROSS` joins, `DROP COLUMN`/`RENAME`, views, triggers. Each exclusion is deliberate (Design §7, §14). The bar for adding a construct: agents writing conservative SQL emit it routinely AND it executes within the existing engine shape (in-memory algebra after pruning, one CAS per write) — `HAVING`, `DISTINCT`, scalar functions, `CASE`, and `ON CONFLICT` upsert cleared it first; uncorrelated subqueries, 3+ table joins/self-joins, and additive `ALTER TABLE … ADD COLUMN` cleared it in 2.5 (inner-query-first rewrite, left-deep hash joins, schema-only commit + read-time NULL fill; additive schema drift auto-migrates at connect); secondary indexes cleared it in 2.6. Correlated subqueries never will.
