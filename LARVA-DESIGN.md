...
larva/
  ...
  tables/
    users/
      ...
      index_email_01H9XKS3....json  ← secondary-index blob: chunkId → sorted distinct values (Section 8)
    orders/
...
Each table is stored chunk-sorted by primary key. Declaring `partitionBy()` on one column (almost always a timestamp or a tenant/user id) additionally maintains zone-map statistics for that column, making range and equality filters on it prune aggressively. Queries filtering on any other column scan the table — unless the column is **indexed** (shipped 2.6, closing the Section 14 roadmap).

**Secondary indexes.** Exactly as promised: an index is just another immutable blob committed in the same atomic swap. Per indexed column, one ULID-named blob maps `chunkId → sorted distinct non-NULL values`; the manifest's table entry carries the current ref, and every commit that changes the table's chunk set re-stages the blob copy-on-write (drop retired ids, add entries for new chunks) and re-points the ref in the same commit — plans, transactions, group-commit batches, and tier-A folds all route through one maintenance hook. Queries with `=`, `IN`, or range predicates on an indexed column then keep only the chunks whose entry can satisfy the predicate (binary search over the sorted values).

The load-bearing decision is what the blob is keyed by. Because entries are keyed by **immutable chunk id**, an entry can only ever be wrong by *absence* — a chunk id missing from the map is simply always fetched — and absence is exactly what every failure mode produces: a pre-index client writing chunks without maintaining the index, an index update lost to a rebase against a concurrent winner, a blob deleted by an older client's vacuum (`readIndex` treats 404 as "no index"). Each degrades pruning; none can return a wrong result. **That is why indexes need no format bump**: pre-2.6 clients preserve the refs they don't understand (their apply functions only touch `chunks`), ignore the index deltas in log entries, and at worst leave the index stale — safe by the same argument, and verified offline by a chaos suite that compares indexed lookups against full scans under contention, plus a planted proto-level "old client" write.

Declaration is `.index()` in `defineSchema` or `CREATE INDEX [name] ON table (column)` / `DROP INDEX ON table (column)` in SQL (one index per column, addressed by column — a given name parses and is ignored; `CREATE UNIQUE INDEX` and composite indexes are rejected by name). Index flags are performance metadata, never drift: connect syncs them both ways, and creating one backfills from the existing chunks in a single commit that re-executes if the chunk set moves underneath it. The primary key and the `partitionBy()` column never need one — zone maps already cover them.
